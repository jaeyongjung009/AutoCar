...
// 필터링된 IMU 자기장 데이터를 수신하고 CSV 파일에 기록하는 콜백 함수
...
int main(int argc, char** argv) {
    // ROS 노드 초기화
    ros::init(argc, argv, "imu_mag_filtered_logging");
    ros::NodeHandle nh;

    // CSV 파일 열기 (파일명을 필요에 따라 변경 가능)
    csv_file.open("imu_filtered_mag_data.csv");

    // CSV 파일에 헤더 작성
    csv_file << "Timestamp,MagneticField_x,MagneticField_y,MagneticField_z\n";

    // /imu/filtered_mag_data 토픽을 구독
    ros::Subscriber sub = nh.subscribe("/imu/filtered_mag_data", 1, magCallback);
