...
class PurePursuit
{
  public:
    ...
    PurePursuit();
    // 차량의 현재 위치와 웨이포인트에 따라 명령을 생성합니다.
    void cmd_generator(const nav_msgs::Odometry::ConstPtr& odom);
    // 계획된 경로를 구독합니다.
    void planned_path_listener(const nav_msgs::Path::ConstPtr& path);
    // pose를 base_link로 변환합니다.
  ...
  private:
    ...
    ros::Subscriber sub_odom_, sub_planned_path_;
                             ...
                             map_frame_id_("map"), robot_frame_id_("base_link"), lookahead_frame_id_("lookahead")
{
  ...
  sub_planned_path_ = nh_.subscribe("/planned_path", 1, &PurePursuit::planned_path_listener, this);
  sub_odom_ = nh_.subscribe("/utm_odom", 1, &PurePursuit::cmd_generator, this);
...
void PurePursuit::cmd_generator(const nav_msgs::Odometry::ConstPtr& odom)
{
  if (path_loaded_)
  {
    // 현재 pose를 가져옵니다.
    ...
    try
    {
      ...
      // 현재 pose와 경로 정보를 바탕으로 추적할 웨이포인트를 결정합니다.
      for (idx_=idx_memory; idx_ < planned_path_.poses.size(); idx_++)
      {
        if (distance(planned_path_.poses[idx_].pose.position, tf.transform.translation) > lookahead_distance_)
        {
          KDL::Frame pose_offset = trans2base(planned_path_.poses[idx_].pose, tf.transform);
      ...
      // 목표에 접근하고 있는 경우 (마지막 웨이포인트)
      if (!planned_path_.poses.empty() && idx_ >= planned_path_.poses.size())
      {
        KDL::Frame goal_offset = trans2base(planned_path_.poses.back().pose, tf.transform);

        // 목표 도달
        if (fabs(goal_offset.p.x()) <= position_tolerance_)
        {
          goal_reached_ = true;
          planned_path_ = nav_msgs::Path(); // 경로를 초기화합니다.
        }
        // 위치 허용 오차에 도달하지 못한 경우: 목표를 넘어서서 lookahead 거리를 연장합니다.
        else
        {
          // 현재 pose를 중심으로 하고, 반지름이 lookahead_distance인 원과 마지막 웨이포인트로 정의된 선의 교차점을 찾습니다.
          double roll, pitch, yaw;
          goal_offset.M.GetRPY(roll, pitch, yaw);
          double k_end = tan(yaw); // 마지막 웨이포인트로 정의된 선의 기울기
      ...
      if (!goal_reached_)
      {
        ...
        lookahead_marker_.pose.position.x = planned_path_.poses[idx_].pose.position.x;
        lookahead_marker_.pose.position.y = planned_path_.poses[idx_].pose.position.y;
        lookahead_marker_.pose.position.z = planned_path_.poses[idx_].pose.position.z;
...
void PurePursuit::planned_path_listener(const nav_msgs::Path::ConstPtr& new_path)
{ 
  if (new_path->header.frame_id == map_frame_id_)
  {
    ...
    {
      std::cout << "Received Planned Path" << std::endl;
      std::cout << "angle = "<< cmd_acker_.drive.steering_angle_velocity << std::endl;
